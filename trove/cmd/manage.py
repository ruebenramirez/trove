...
class Commands(object):
    ...
    def datastore_version_update(self, datastore_id_or_name, version_name, manager,
    ...
    def add_capability(self, name, description, enabled):
        ...
        try:
            ...
            print 'unable to create the capability:', e

    def remove_capability(self, capability_id_or_name):
        """Remove capability and all children capability overrides"""
        try:
            capability = datastore_models.Capability.load(capability_id_or_name)
            capability.delete()
        except Exception as e:
            print 'unable to remove capability:', e

    def add_capability_override(self, capability_id, datastore_version_id, enabled):
        """Adds capabilty override to a datastore"""
        try:
            datastore_models.CapabilityOverride.create(capability_id, datastore_version_id, enabled)
        except Exception as e:
            print 'Unable to add capability override:', e

    def remove_capability_override(self, capability_id, datastore_version_id):
        """Removes capability override on a datastore version"""
        try:
            capability_override = datastore_models.CapabilityOverride.load(capability_id, datastore_version_id)
            capability_override.delete()
        except Exception as e:
            print 'Unable to remove capability override:', e

    def remove_capability_overrides(self, capability_id, enabled='everything'):
        """Remove overrides for a capability"""
        try:
            datastore_models.CapabilityOverrides.delete_capability_overrides(capability_id, enabled)
        except Exception as e:
            print 'Unable to remove capability overrides:', e
